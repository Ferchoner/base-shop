...
- ADR-0081 aceptado: P-67 cerrada. En el MVP hay exactamente un almacén, creado por el seed; se retiran de `API_SPEC.md` la creación y la desactivación de almacenes; índice único parcial de almacén activo en `DATABASE.md` (modifica ADR-0066); BR-INV-08 y UC-INV-01 actualizados. Confirmadas las interpretaciones de ADR-0080 (el producto sigue mostrando su marca inactiva; la búsqueda no usa nombres de categorías ocultas).
- Agregado ADR-0083: plazo de entrega estimado en días hábiles desde la confirmación del pago, configurable en el método de envío (inicial 3 a 7), visible en la cotización, en el correo de orden recibida y como snapshot en la orden; sin plazo interno de despacho en el MVP. P-64 cerrada; `shipping_methods` y `orders` agregan `delivery_min_business_days` y `delivery_max_business_days` (modifica ADR-0066); la validación legal de su presentación se suma a P-61.
- Agregado ADR-0084: Prettier para código y configuración (sin Markdown, `.prettierignore`), scripts `format` y `format:check`; ramas `tipo/T-xxx-descripcion`; Conventional Commits; ramas, commits y pull requests en inglés desde el 2026-09-26, sin reescribir el historial. P-70 cerrada; T-104 sin decisiones pendientes.
